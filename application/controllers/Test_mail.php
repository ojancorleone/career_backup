...
class Test_mail extends CI_Controller {
			...
			function sendMail()  
			{
				...
				$url = base_url().'register/confirm_email/'.base64_encode($this->encryption->encrypt('1')).'/'.base64_encode($this->encryption->encrypt('bayu.wijaya06@gmail.com'));
			...
			function cek_array(){
				...
				$array_json = json_decode($var_json , TRUE);


				if (is_array($array_json['resource'])) {
					foreach ($array_json['resource'] as $key => $value) {
						echo $value['m_vacancy_stages_id']."<br>";
					}
				}else{
						echo "here";
				}
