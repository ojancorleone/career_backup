...
class M_applicant_list extends CI_Model {
	...
	public function list_data($length,$start,$search,$order,$dir,$column_name){
		...
		$filter="";
		 if($search!=""){
			$filter="and((fullname like '%".$search."%')OR(email like '%".$search."%')OR(company_name like '%".$search."%')OR(job_position like '%".$search."%')OR(job_category like '%".$search."%'))";
		}
		...
		$result_data = $this->Api->get('_table' , 'vw_applicants' , $fields , $filter, $order_by , $limit, $offset);

		$get_total_row	= $this->Api->get('_table' ,'vw_applicants', NULL, $filter , NULL, NULL, NULL, TRUE);
	...
	public function list_data2($length,$start,$search,$categories){
		...
		if(strlen($cats)>0){
			...
			$filter="((job_category_id IN ($cats)))";
		}else{
			$filter="((job_category_id IS NULL))";
		}

		 if($search!=""){
			$filter.="AND((fullname like '%".$search."%')OR(email like '%".$search."%')OR(company_name like '%".$search."%')OR(job_position like '%".$search."%'))";
		}
		...
		$result_data = $this->Api->get('_table' , 'vw_hiring_step' , $fields , $filter, $order_by , $limit, $offset);

		//print_r($result_data);

		$get_total_row	= $this->Api->get('_table' ,'vw_hiring_step', NULL, $filter , NULL, NULL, NULL, TRUE);
		$total_row		= $get_total_row;
	...
	public function sum_category()
	{
		$result_data	= $this->Api->get('_table' ,'vw_applicant_category_summary', '*');
		return $result_data['resource'];
	}
