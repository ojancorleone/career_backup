...
class Master_setting extends CI_Model {
	...
	public function SetSettingSMTP()
	{
		$type 		= '_table';
		$type_val 	= 'mail_setting';
		$fields 	= 'protocol,smtp_host,smtp_port,smtp_user,smtp_from,smtp_fromname,smtp_pass,mailtype,charset,word_wrap';
		$filter 	= NULL;
		$order 		= NULL;
		$limit 		= 1;
		$setting 	= $this->Api->get($type, $type_val, $fields, $filter, $order, $limit);
		return $setting['resource'][0];
	}
	...
	public function check_auth($level=NULL, $url=NULL)
	{
		...
		$type_val	= "m_menu";
		$fields 	= "id";
		$filter 	= "and(userlevel like '%\"".$level."\"%')and(url=".$url.")";
		$menus 		= $this->Api->get($type,$type_val,$fields,$filter);
		...
		if (!empty($menus['resource'][0]['id'])) {
			return TRUE;
		}else{
			return FALSE;
		}

		// return $menus['resource'][0];
	}

	public function SetMenuFrontend(){
		$type 		= "_table";
		$type_val	= "m_menu";
		$fields 	= "id,menuname,parent,url";
		$filter 	= "(status=1)and(frontend=1)";
		$order 		= NULL;
		$menus 		= $this->Api->get($type,$type_val,$fields,$filter,$order);
		return $menus['resource'];
	}
